...
function drawBlips()
	for k,v in ipairs(getElementsByType("blip")) do
		if not getElementData(v,"DoNotDrawOnMaximap") then
			...
			local size=(getBlipSize(v) or 2)*8
...
addEventHandler("onClientKey", root, function(key, por)
	if key == "F11" then
		toggleMap()
		cancelEvent()
	end
...
function toggleMap()
	if toggle then
		if triggerEvent("onClientPlayerMapHide",getRootElement(),false) then
			toggle=false
			setElementData(localPlayer,"TuningMenuTrue",false)
		end
	else
		if triggerEvent("onClientPlayerMapShow",getRootElement(),false) then
			toggle=true
			setElementData(localPlayer,"TuningMenuTrue",true)
		end
	end
end
...
function setPlayerMapImage(image,tLX,tLY,lRX,lRY)
	...
	elseif not image then
		...
		mapFile                           =":aps-f11/images/radar.png"
