...
RobotoL = DxFont("files/light.ttf", 11);
RobotoL2 = DxFont("files/light.ttf", 8);
RobotoL3 = DxFont("files/light.ttf", 10);
RobotoR = DxFont("files/regular.ttf", 10);
RobotoR2 = DxFont("files/regular.ttf", 13);
FontIcons = DxFont("files/icons.otf", 10);
FontIcons2 = DxFont("files/icons.otf", 7);
FontIcons3 = DxFont("files/icons.otf", 7);
...
local details = {
    ...
    ["success"] = {"Successo", "success", "s.mp3"},
}
...
function getColors(render)
    ...
    if enabled then
        ...
        types = {
            ...
            ["info"] = {"", {engine:getServerColor('blue2', false)}},
...
function drawnBoxes()
    ...
    for k,v in ipairs(cache) do
        ...
        if state == "fadeIn" then
            ...
            if progress < 0 then
                pos = {interpolateBetween(100 - 80, sy - 305 - (47 * 1), 0, 20, sy - 305 - (47 * 1), 0, progress, 'OutQuad')}
        ...
        local boxWidth = length + 97
        dxDrawRectangle(pos[1] + 00 - 09, pos[2] + 90 + 40, boxWidth, 38, tocolor(r,g,b, alpha[1]),true)

        
        if timeLine then
            boxWidth = interpolateBetween(boxWidth, 0,0, 0, 0,0, timeLineProg or 1, customProg and "Linear" or 'OutQuad')
        else
            boxWidth = boxWidth
        end
        dxDrawRectangle(pos[1] + 00 - 09, pos[2] + 90 + 40, boxWidth, 38, tocolor(255, 255, 255, 50),true)
        
        nowY = sy - 305 - (47 * 1) + 35
        dxDrawText(msg, pos[1] + 09 - 09, pos[2] + 99 + 40, boxWidth, 38, tocolor(255, 255, 255, alpha[2]), 0.75, RobotoR2, 'left', 'top',false,false,true,true,false)
