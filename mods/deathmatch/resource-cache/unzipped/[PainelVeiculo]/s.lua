...
function reparar()
	...
	if isPedInVehicle(source) then
		fixVehicle(veiculo)
		displayServerMessage(source, "Você Reparou Seu Veiculo", "confirm")
	end
end
...
function virar()
	...
	if isPedInVehicle(source) then
		...
		displayServerMessage(source, "Você Virou Seu Veiculo", "confirm")
	end
end
addEvent("virar", true)
...
function nitro()
	...
	if isPedInVehicle(source) then
		...
		displayServerMessage(source, "Você Ativou O Nitro", "confirm")
	end
end
addEvent("nitro", true)
...
displayServerMessage(source, "O Motor Do Seu Veiculo Foi Desligado", "warning")
end
addEvent("m1", true)
addEventHandler("m1", root, m1)

function motor1 ()
local vehicle = getPedOccupiedVehicle(source)
setVehicleEngineState ( vehicle, true )
displayServerMessage(source, "O Motor Do Seu Veiculo Foi Ligado", "confirm")
end
addEvent("motor1", true)
addEventHandler("motor1", root, motor1)

function l1 ()
local vehicle2 = getPedOccupiedVehicle(source)
setVehicleOverrideLights ( vehicle2, 2 )
displayServerMessage(source, "Luzes Ligadas", "confirm")
end
addEvent("l1", true)
addEventHandler("l1", root, l1)

function l2 ()
local vehicle1 = getPedOccupiedVehicle(source)
setVehicleOverrideLights ( vehicle1, 1 )
displayServerMessage(source, "Luzes Desligadas", "warning")
end
addEvent("l2", true)
addEventHandler("l2", root, l2)


function t1 ()
displayServerMessage(source, "Veiculo Destrancado", "warning")
...
addEvent("t1", true)
addEventHandler("t1", getRootElement(), t1)

function t2 ()
displayServerMessage(source, "Veiculo Trancado", "confirm")
	local vehicles = getElementsByType ( "vehicle" )
	for k,v in ipairs(vehicles) do
...
addEvent("t2", true)
addEventHandler("t2", getRootElement(), t2)


function l4 ()
displayServerMessage(source, "Freio De Mão Ativado", "confirm")
local vehicle5 = getPedOccupiedVehicle(source)
setElementFrozen ( vehicle5 , true )
end
addEvent("l4", true)
addEventHandler("l4", getRootElement(), l4)

function l3 ()
displayServerMessage(source, "Freio De Mão Desativado", "warning")
local vehicle4 = getPedOccupiedVehicle(source)
setElementFrozen ( vehicle4 , false )
end
addEvent("l3", true)
addEventHandler("l3", getRootElement(), l3)
